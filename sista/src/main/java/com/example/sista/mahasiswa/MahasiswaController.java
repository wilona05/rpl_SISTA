...
public class MahasiswaController {
    ...
    @GetMapping("/catRevisi")
    public String getCatatanRevisi(@RequestParam("id") int id, Model model) {
        // Fetch the catatan revisi for the given Sidang ID
        String catatanRevisi = repoSidang.getCatatanRevisiById(id);

        // Check if no revisi was found
        if (catatanRevisi == null || catatanRevisi.isEmpty()) {
            model.addAttribute("errorMessage", "No revisions found for the selected Sidang.");
            return "error-page"; // Replace with your error page
        }

        // Add the catatan revisi to the model
        model.addAttribute("isiRevisi", catatanRevisi);
        return "mahasiswa/catatanRevisi"; // Corresponding Thymeleaf template
    }
